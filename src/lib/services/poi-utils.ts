...
export function computeByMethod(donationList: Donation[]) {
  donationList.forEach((donation) => {
    if (donation.method == "paypal") {
      currentDataSets.placemarkersByDifficulty.datasets[0].values[0] += donation.amount;
    } else if (donation.method == "direct") {
      currentDataSets.placemarkersByDifficulty.datasets[0].values[1] += donation.amount;
    }
  });
}
